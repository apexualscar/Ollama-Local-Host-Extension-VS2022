...
namespace OllamaLocalHostIntergration
{
    public partial class MyToolWindowControl : UserControl
    {
        ...
        /// <summary>
        /// Loads code templates into the dropdown
        /// </summary>
        private void LoadTemplates()
        {
            ...
            var items = new List<object> { "📝 Quick Templates..." };
            items.AddRange(templates);
            
            comboTemplates.ItemsSource = items;
        ...
        /// </summary>
        private async void ComboTemplates_SelectionChanged(object sender, SelectionChangedEventArgs e)
        {
            if (_isInitializing || comboTemplates.SelectedIndex <= 0)
                return;

            try
            {
                if (comboTemplates.SelectedItem is CodeTemplate template)
                {
                    // Get code selection if template requires it
                    string code = null;
                    string language = "";
                    
                    if (template.RequiresCodeSelection)
                    {
                        code = await _codeEditorService.GetSelectedTextAsync();
                        if (string.IsNullOrEmpty(code))
                        {
                            txtStatusBar.Text = "Please select code to apply this template";
                            comboTemplates.SelectedIndex = 0;
                            return;
                        }
                        language = await _codeEditorService.GetActiveDocumentLanguageAsync();
                    }

                    // Build prompt from template
                    string prompt = _templateService.BuildPromptFromTemplate(template.Id, code);
                    
                    // Set recommended mode
                    if (template.RecommendedMode == InteractionMode.Agent)
                    {
                        _modeManager.SwitchToAgentMode();
                        comboMode.SelectedIndex = 1;
                    }
                    ...
                    // Add user message
                    var userChatMessage = _messageParser.ParseMessage(prompt, true);
                    _chatMessages.Add(userChatMessage);
                    _currentConversation.Messages.Add(userChatMessage);
                    ...
                    var streamingMessage = new ChatMessage("", false);
                    _chatMessages.Add(streamingMessage);
                    txtStatusBar.Text = $"Applying template: {template.Name}...";

                    // Get response with template's system prompt
                    string fullResponse = await _ollamaService.GenerateStreamingChatResponseAsync(
                        prompt,
                        token =>
                        {
                    ...
                    if (_modeManager.IsAgentMode && responseChatMessage.HasCodeBlocks)
                    {
                        try
                        {
                            var codeEdit = await _codeModService.CreateCodeEditFromResponseAsync(fullResponse, code);
                            if (codeEdit != null)
                            {
                                responseChatMessage.AssociatedCodeEdit = codeEdit;
                                responseChatMessage.IsApplicable = true;
                    ...
                    txtStatusBar.Text = $"Template applied: {template.Name}";
                    chatMessagesScroll.ScrollToBottom();
                }
            }
            catch (Exception ex)
            {
                txtStatusBar.Text = $"Error applying template: {ex.Message}";
            }
            finally
            {
                // Reset selection
